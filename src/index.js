...
async function start() {
    ...
    if (!Matrix.authState.creds.registered && isSessionPutted) {
        const sessionID = config.SESSION_ID.split('MIRAGE-MD&')[1];
...
Matrix.ev.on("connection.update", async update => {
    ...
    if (connection === "open") {
        if (initialConnection) {
            ...
            Matrix.sendMessage(Matrix.user.id, { text: `😃 MIRAGE-MD EDITX CONNECTED SUCESSFULY ENJOYYY✅` });
